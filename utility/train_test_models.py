...
def train_model():
    ...
    # source = "/Users/mikolajszczesny/VSCode/python/Speaker-Identification-Using-Machine-Learning/training_set"
    ...
    for path in file_paths:
        ...
        sr, audio = read(training_source + path)
        print(sr)
        ...
        print(count)
        if count == name_sample_count[speaker_id]:
            gmm = GaussianMixture(
                n_components=6,
                max_iter=200,
            ...
            pickle.dump(gmm, open(training_dest + picklefile, "wb"))
            print(
...
def test_model():
    ...
    gmm_files = [
        os.path.join(training_dest, fname)
        for fname in os.listdir(training_dest)
    ...
    for path in file_paths_test:
        ...
        path = path.strip()
        # print(path)
        sr, audio = read(testing_source + path)
        vector = extract_features(audio, sr)
        ...
        print(f"\t{path} detected as - {speakers[winner]}")
        # time.sleep(1.0)
        winners.append(speakers[winner])
    return in_files, winners


def get_models():
    models = []
    for model in os.listdir("./trained_models"):
        if model.endswith(".gmm"):
            models.append(model)
    return models
...
# while True:
#     test = DatasetManipulation.extract_name_count(training_source)
#     print(test)
#     choice = int(
#         input(
#             "\n 1.Record audio for training \n 2.Train Model \n 3.Record audio for testing \n 4.Test Model\n"
...
if __name__ == "__main__":
    from utils import DatasetManipulation

    print("Bliblioteka, nie uruchamiac!")
else:
    from utility.utils import DatasetManipulation
